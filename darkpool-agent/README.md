...
输入 A股代码，自动操作安卓手机上的同花顺 App，进入该股票个股页的「资金」栏目，
截取**主力暗盘**板块（同花顺付费功能：统计大资金通过量化拆单隐藏的买卖资金，
显示为「主力明盘 / 主力暗盘」柱状图），通过 HTTP 接口返回图片，供小程序聊天框发送给客户。
...
输出 JSON 动作（0~1000 相对坐标），直到确认目标股票「资金」栏目的主力暗盘板块
完整可见，再截最终图返回。
...
- **自动跑批**：每天到 `PREFETCH_AT`（默认北京时间 **15:30**，收盘 15:00 后）服务
  内置定时器自动跑批——先让智能体打开同花顺**自选股**列表页、用 GLM-4V 读出
  列表里全部股票代码，再逐只进入「资金」栏目截取主力暗盘板块，存入本地按天缓存
  `cache/YYYYMMDD/`。要给客户提供哪些股票，把它们加进 App 自选股即可；
  也可用 `PREFETCH_CODES=600519,300750` 固定代码列表跳过自动识别。
- **当天缓存**：`/api/darkpool/screenshot` 先查当天缓存，命中直接返回（毫秒级），
  未命中才操作真机；同一代码的并发请求会合并成一次真机操作。
- **手动触发/查进度**：
  ```bash
  # 手动跑批（codes 省略则自动读取App自选股列表）
...
2. App 内已登录**开通了「暗盘」功能的同顺会员付费账号**（本工具不会、也不能绕过付费）；
...
#   export PREFETCH_AT=15:30                # 每日自动跑批时间(收盘15:00后)，空串关闭
#   export PREFETCH_CODES=600519,300750     # 固定跑批代码；不设则自动读取App自选股列表
...
返回 `{"ok":true,"code":"600519","cached":false,"image_base64":"..."}`。
...
- 主力暗盘是同花顺的**付费数据服务**。请使用自己已付费开通的账号，并自行确认
  你与同花顺的用户协议 / 数据授权是否允许将数据截图转发给客户（尤其是商业用途、
  面向不特定多数人分发的场景）。
