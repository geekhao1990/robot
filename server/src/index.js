...
const { processDueAiWorkOrders } = require('./dark-fund-orders');
...
let workOrderProcessing = false;

async function completeDueWorkOrders() {
  if (workOrderProcessing) return;
  workOrderProcessing = true;
  try {
    const result = processDueAiWorkOrders(db.get());
    if (result.prepared || result.completed) {
      db.save();
      await db.flush();
    }
  } catch (error) {
    console.error('自动完成暗盘工单失败', error);
  } finally {
    workOrderProcessing = false;
  }
}

db.load()
  .then(async () => {
    await completeDueWorkOrders();
    workOrderTimer = setInterval(completeDueWorkOrders, 5000);
    workOrderTimer.unref();
...
async function shutdown() {
  if (workOrderTimer) clearInterval(workOrderTimer);
  server.close(async () => {
